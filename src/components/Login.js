...
export default class Login extends Component {
  ...
  componentDidMount() {
    let localAdmin = localStorage.getItem("isAdmin");
    if (localAdmin !== null) {
      if (localAdmin === "true") {
  ...
  handleLogin = (event) => {
    ...
    let admin = {isAdmin: true, username: "admin2", password: "admin2",firstname:"Jessa", middlename:"Hortado", lastname:"Yosores", profession:"admin",haschange: true }
    let body = { username: this.state.username, password: this.state.password };
    req
      .login(body)// to save admin >>> fakelogin(admin)
      .then(resp => {
        if (resp.data.status) {
